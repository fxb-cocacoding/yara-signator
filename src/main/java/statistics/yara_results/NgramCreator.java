...
public class NgramCreator {
	...
	public List<Ngram> getNgramsForFamily_NextGen_CandidateOne(int family_id, Config config, NextGenConfig currentNextGen) throws SQLException, DecoderException, IOException {
		...
		for(int sample_id: sample_ids) {
			...
			while(rs.next()) {
				...
				if(IterativeImprovementDataContainer.INSTANCE.blackListedSequenceCandidates.contains(ngram.getOpcodes().toLowerCase())) {
	...
	private static Integer getSampleIdFromSampleFilename(String filename) throws SQLException, UnexpectedException {
		...
		if(rs.getFetchSize() > 1) {
			...
			throw new UnexpectedException("We should not have more than two samples with the same filename int the corpus...");
